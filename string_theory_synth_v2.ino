...
/* Original code by Jan Ostman,  Modified by Dave Field of Make Synths Not War

        https://makesynthsnotwar.com
    ...
    *****
    ...
    1. MIDI offset function added
    2. MIDI note handlers refactored
    3. STANDALONE VERSION: MIDI Thru enabled
    ...
    *****
    ...
    3. MIDI handler re-written to use stock MIDI library and to be channel-aware
    4. MIDI "learn" feature added to allow MIDI channel to be set without code change
    5. EEPROM library added to allow MIDI channel to be stored
    6. Any resulting redundant code and variables removed
...
// Analog Pins
const byte ENVELOPE_PIN = 5;
const byte ENSEMBLE_PIN = 4;
const byte MODULATION_PIN = 3;

// Digital Pins
const byte MIDI_LEARN_PIN = 12;
const byte AUDIO_OUT_PIN = 11;
const byte GATE_OUT_PIN = 10;
const byte LED_OUT_PIN = 9;

// MIDI LEARN Mode
const byte MIDI_LEARN_LED_PERIOD_MS = 10;
bool learnModeActive = false;
byte learnStep = 1;
byte learnNote = 0;
byte learnButtonState = HIGH;
byte learnLedLastState = LOW;
long learnLedLastFlash = 0;
...
void setup() {
  ...
  pinMode(MIDI_LEARN_PIN, INPUT_PULLUP);
  ...
  // Version 2 has mods to the EEPROM handling
  // EEPROM addresses
  //
  // 0 - midiChannel (byte)
  // 1, 2, 3, 4 - midiOffset (int)
  // 5 - check this location to see if EEPROM has been initialised

  // Read EEPROM for initialisation
  byte eepromValue;
  eepromValue = EEPROM.read(5);
  if (eepromValue == 255) {
    // never previously written at software v2 - write defaults
    EEPROM.write(0, 1); // default MIDI channel = 1
    EEPROM.put(1, 0); // default MIDI offset = 0;
    EEPROM.write(5, 0); // initialisation flag
  }

  // Read EEPROM for MIDI channel & offset
  midiChannel = EEPROM.read(0);
  EEPROM.get(1, midiOffset);
...
void loop() {
  ...
  while (1) {
    ...
    // read "Learn mode" button
    int state = digitalRead(MIDI_LEARN_PIN);
    if (!state && learnButtonState) // same as if(button == LOW && oldbutton == HIGH)
    {
      learnButtonState = state;
      learnModeActive = true;
    }
    else if (state && !learnButtonState) // same as if(button == HIGH && oldbutton == LOW)
    {
      // the button was released
      learnButtonState = HIGH;
    }

    // flicker the gate led if we're in learn mode
    if (learnModeActive) {
      long now = millis();
      if (now > (learnLedLastFlash + MIDI_LEARN_LED_PERIOD_MS)) {
        learnLedLastState = !learnLedLastState;
        digitalWrite(LED_OUT_PIN, learnLedLastState);
        learnLedLastFlash = now;
      }
    }
    else {
      digitalWrite(LED_OUT_PIN, TRIG);
    }
    digitalWrite(GATE_OUT_PIN, TRIG);
...
void midiNoteOnHandler(byte channel, byte note, byte velocity) {
  if (learnModeActive) {
    if (learnStep == 1) {
      ...
      midiOffset = learnNote - note;
      EEPROM.put(1, midiOffset);
